...
| 항목 | 상태 |
| --- | --- |
| 브라우저 영상 재생 방식 | WebRTC로 확정([결정 0027](../../docs/architecture/decisions.md#0027--실시간-관제-전달을-httpwebrtcsse로-구성한다)) |
| 실시간 화면 갱신 방식 | SSE로 확정(같은 결정) |
| 영상 저장 범위·보존 기간·접근 권한 | `결정 필요` (개인정보 관련 합의 사항) |
| 운영 접근 통제 방식 | MVP 동안 미도입([결정 0030](../../docs/architecture/decisions.md#0030--실시간-영상-접근-제어와-운영-배포를-mvp-동안-인증-최소화로-정한다)). **정해지기 전까지 실시간 영상을 운영에 노출하지 않는다** |
| worker의 영상 수신 프로토콜과 MediaMTX 경로·인증 | 인증 없음 + 카메라 ID 경로([결정 0030](../../docs/architecture/decisions.md#0030--실시간-영상-접근-제어와-운영-배포를-mvp-동안-인증-최소화로-정한다)) |
| MediaMTX 설정 위치 | 설정·문서만 이 디렉터리에 둠([결정 0030](../../docs/architecture/decisions.md#0030--실시간-영상-접근-제어와-운영-배포를-mvp-동안-인증-최소화로-정한다)) |
...
[결정 0027](../../docs/architecture/decisions.md#0027--실시간-관제-전달을-httpwebrtcsse로-구성한다)이 정본이다.
...
[결정 0027](../../docs/architecture/decisions.md#0027--실시간-관제-전달을-httpwebrtcsse로-구성한다)의
예외에 따라, fastapi가 허용한 WebRTC 세션에 한해 브라우저가 MediaMTX와
signaling·미디어 연결을 맺는다. 브라우저가 worker나 deeplearning을 직접 호출하는
것은 계속 금지한다.

[결정 0030](../../docs/architecture/decisions.md#0030--실시간-영상-접근-제어와-운영-배포를-mvp-동안-인증-최소화로-정한다)에
...
**설정·문서만 두는 쪽으로 정했다**
([결정 0030](../../docs/architecture/decisions.md#0030--실시간-영상-접근-제어와-운영-배포를-mvp-동안-인증-최소화로-정한다)).
