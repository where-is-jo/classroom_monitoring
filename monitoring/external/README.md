...
"스마트 오피스 모니터링"이라는 주제 그 자체에 해당하는 쪽이고, 보는 사람은 팀이 아니라
서비스 사용자다.
...
루트 `CLAUDE.md`의 최상위 구조 제약은 `monitoring/`을 설정 디렉터리로 규정해 왔고,
...
- 사람·수화기 탐지 등 추론 — `deeplearning`이 맡는다
- 탐지 결과의 비즈니스 해석(재실/부재 판정) — `fastapi`가 맡는다
...
- `webapps/fastapi`: 화면과 인증·권한을 담당한다. **누가 어떤 카메라를 볼 수 있는지는
