...
def test_AdaFace_모델은_고정_revision에서_생성하고_전송_hash만_확인한다() -> None:
    workflow = WORKFLOW_PATH.read_text(encoding="utf-8")
    ...
    assert "MODEL_SHA256_ALLOWLIST" not in workflow
    assert (
        prepare_position
        < local_hash_position
        < transfer_position
...
def test_변환_스크립트는_ONNX_산출물_SHA를_게이트로_쓰지_않는다() -> None:
    source = (
        REPOSITORY_ROOT / "deeplearning" / "training" / "prepare_adaface_model.py"
    ).read_text(encoding="utf-8")

    assert "ONNX_FILE_SHA256" not in source
