"""GPU 호스트에서 사람 탐지 모델 계약을 독립적으로 검증한다.

GPU 배포는 서버 호스트에 ``.docker``만 동기화하고 worker 소스는 Docker 이미지
안으로만 보낸다. 따라서 배포 사전점검은 worker 패키지를 import하지 않고 이 모듈만으로
실행되어야 한다.
"""
...
def verify_person_model_contract(
    ...
    """가중치·클래스·image size·전처리 계약이 배포 설정과 맞는지 확인한다."""
    ...
    if _sha256_file(model) != expected_hash:
    ...
    if raw.get("image_size") != image_size:
        raise ValueError("INFERENCE_IMAGE_SIZE가 모델 계약과 다릅니다.")
