...
4. 프로젝트 디렉터리와 README를 먼저 만든다. README에는 다음을 포함한다.
   ...
   - 테스트 전략
   - 관련 문서 링크
5. 기술 선택 중 되돌리기 비용이 큰 것은 `docs/architecture/decisions.md`에 항목으로 남긴다.
6. 최소한의 디렉터리 골격만 만든다. 실행 코드는 이번 범위가 아니다.
7. 루트 `README.md`에 새 프로젝트를 반영해야 하는지 확인한다.

## 금지 사항
- 최상위에 새 파일·디렉터리를 만들지 않는다. 허용된 최상위 항목은
  `webapps/`, `deeplearning/`, `worker/`, `monitoring/`, `docs/`, `RPAs/`, `README.md`뿐이다.
  (웹 요청을 처리하지 않는 새 서비스를 추가하는 경우는 예외이며, 이때 AGENTS.md와
  루트 README의 구조 표를 함께 갱신한다.)
