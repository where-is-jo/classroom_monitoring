...
> 현재 사람 행위자와 MongoDB metadata 경계는 구현돼 있다. CCTV·추론·MinIO 영상·외부 알림·
> RPA 연동은 아직 구현되지 않았으며 실제 연동 대상은 확정 시 갱신한다.
...
flowchart TB
    ...
    STUDENT -->|직원·면담·좌석| SYS
    STAFF -->|면담 처리·본인 상태·좌석| SYS
    ADMIN -->|사용자·운영 데이터 관리| SYS
...
| --- | --- | --- |
| 학생 (`STUDENT`) | 직원 상태와 강의실 좌석 조회, 본인 면담 신청·처리 | 구현됨 |
...
제품 권한 모델은 위 세 역할이다. 저장 데이터 역직렬화를 위한 레거시 역할의 경계는
[ADR-0006](./decisions/ADR-0006-v2-legacy-compatibility.md)에 있다.
...
| MongoDB | 운영 메타데이터와 설정을 보관한다 | 구현됨 ([ADR-0003](./decisions/ADR-0003-metadata-store-mongodb.md)) |
