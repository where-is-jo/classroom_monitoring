...
| 단계 | 입력 | 출력 | 담당 |
| --- | --- | --- | --- |
| 1. 수집 | 카메라 영상 | 연속 프레임 | worker |
| 2. 샘플링 | 연속 프레임 | 추론 대상 프레임 | worker |
| 3. 탐지 | 프레임 | 객체·좌표·신뢰도 | deeplearning |
| 4. 해석 | 탐지 결과 | 업무 의미가 부여된 상태 | 소유 서비스 `결정 필요` |
| 5. 저장 | 상태·이벤트 | 저장된 레코드 | fastapi |
| 6. 표시 | 조회 결과 | 화면 | fastapi |

- **2단계에서 모든 프레임을 추론에 보내지 않는다.** 샘플링 주기는 설정값으로 둔다.
- **3단계는 의미를 부여하지 않는다.** `person 1, conf 0.87`까지가 출력이다.
- **4단계에서 처음으로 업무 의미가 생긴다.**

각 단계는 다음 단계가 멈춰도 전체가 무너지지 않게 만든다. 실패 시 프레임을 버릴지
쌓아둘지는 `결정 필요`이며, "쌓아둔다"를 고르면 메모리 한계를 함께 정한다.
