...
실행 가능한 서비스는 **`webapps/fastapi` 하나**다. 세 제품 역할의 캠퍼스 운영 포털이
memory 또는 MongoDB metadata 저장소로 동작한다. 실행·로그인·테스트 방법은
[fastapi README](../../webapps/fastapi/README.md)에 있다.

`deeplearning`, `worker`, `monitoring`, `RPAs`는 책임과 경계 문서만 있고 실행 코드는 없다.
실제 영상 수집·추론 파이프라인이 구현된 것으로 전제하지 않는다.
...
2. `develop`에서 브랜치를 딴다. 기준 브랜치와 PR 대상은 [Git 규칙](../conventions/git-convention.md#브랜치)에 있다.
3. 작업 종류에 맞는 스킬을 연다.
...
**실행 방법이 문서에 없다.**
현재 fastapi 외 서비스에는 구현이 없다. 지어내지 말고, 구현한 사람이 해당 서비스 README에
실제로 확인한 명령을 기록한다.
