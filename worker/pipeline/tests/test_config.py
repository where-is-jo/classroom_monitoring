...
def test_역할별_샘플링_주기는_환경변수로_재정의할_수_있다(
    ...
    monkeypatch.setenv("PERSON_TRACKING_SAMPLE_INTERVAL_FRAMES", "2")
    monkeypatch.setenv("BYTETRACK_BUFFER_FRAMES", "60")

    settings = build_settings()

    assert settings.face_identity_sample_interval_frames == 16
    assert settings.person_tracking_sample_interval_frames == 2
    assert settings.bytetrack_buffer_frames == 60
