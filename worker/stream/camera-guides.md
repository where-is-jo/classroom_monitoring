...
```text
카메라 → FFmpeg → MediaMTX → OpenCV → stream worker
                                          ├→ 프레임 샘플링 → inference
                                          ├→ 원본 영상 저장 (개발용, 기본 꺼짐)
                                          └→ 학습용 프레임 저장 (개발용, 기본 꺼짐)
```

아래 설정값과 문제 대응은 **USB 카메라 1대로 얻은 것**이다. 목표 구성인 어안 CCTV 1대 +
입구 카메라(라즈베리파이 + 웹캠) 1대는 아직 검증하지 않았다([결정 0024](../../docs/architecture/decisions.md#0024--카메라-구성을-전체-조망-cctv와-입구-카메라로-바꾸고-학생-식별을-입구-1회로-한정한다)).
**어안 CCTV는 왜곡 보정 없이는 좌석 판정에 쓸 수 없고, 보정 수행 위치가 `worker/stream`
인지는 아직 `결정 필요`다.**
...
| 다중 카메라 | 워커 하나가 여러 소스를 스레드로 관리 | 구현됨. 실제 2대 이상 검증은 `예정` |
| 강의실 배치 | 어안 CCTV 1대 + 입구 카메라 1대. 입구 얼굴 크기·차폐·역광·두 화각 겹침 확인 | `예정`. 실제 촬영으로 확정한다 |
| 어안 왜곡 보정 | 탐지 이전에 한 번만 수행, 보정 좌표계가 정본 | `예정`. 수행 위치 `결정 필요` |
| 입구 카메라 노드 | 라즈베리파이 + 웹캠 → RTSP 송출 | `예정`. 저장소에 코드가 없다 |
| Jetson 적용 | `카메라 → Jetson → RTSP 네트워크 → 서버` | `예정` |
| 얼굴 인식 연결 | 샘플링한 프레임에서 학생 식별 | `예정`. 모델이 `deeplearning`에 없다 |
| 결과 전달 | 탐지 결과를 fastapi로 전달 | `예정`. HTTP로 확정([결정 0027](../../docs/architecture/decisions.md#0027--실시간-관제-전달을-httpwebrtcsse로-구성한다)) |
...
**얼굴 인식에 필요한 화질을 확인하지 않았다.** 20 FPS·현재 해상도에서 좌석에 앉은
학생의 얼굴이 인식 가능한 크기로 잡히는지는 실제 강의실 촬영으로 확인해야 한다.
작은 얼굴이 문제가 되면 Super Resolution보다 카메라 위치·렌즈·화각을 먼저 조정한다
([MVP 명세의 카메라 구성](../../docs/specs/student-monitoring-mvp.md#카메라-구성)).
