...
> `APP_ENV=prod`에서 로컬 저장소를 거부하는 것, 세그먼트 파일에서 적재·보존 기간
> 삭제까지 실제 컴포넌트로 잇는 통합 테스트.
> **확인하지 못한 것**: 실제 카메라와 FFmpeg을 붙인 녹화, 실제 MinIO 서버 적재.
> 이 환경에 FFmpeg도 MinIO도 `minio` 패키지도 없다. 갖춘 사람이 확인한 뒤 이 문단을 갱신한다.
