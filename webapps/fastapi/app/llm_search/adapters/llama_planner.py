...
`json_schema`를 모르는 빌드는 요청 자체를 4xx로 거절한다. 그러면 기능이 통째로 죽으므로
4xx를 받으면 한 번만 `json_object`로 낮춰 다시 보낸다. 재시도가 아니라 호환성 처리라 이
계층에 둔다 — 모델이 규격을 벗어난 경우의 재시도는 서비스가 담당한다.

**dev GPU 서버에서 확인했다: llama.cpp `b10362`(server-cuda)는 `json_schema`를 200으로
받는다.** 우리가 보내는 실제 요청(`PLAN_JSON_SCHEMA` + 같은 지시문)으로 확인했고, 모델이
낸 계획은 `parse_plan` 검증까지 통과했다. 즉 **이 빌드에서 아래 폴백 경로는 돌지 않는다**
(`classroom_monitoring_llm_schema_fallback_total`이 0으로 남는다).

그래도 폴백을 지우지 않는 이유는 빌드가 바뀔 수 있어서다. 이 지표가 0이 아니게 되면
서버 빌드가 내려갔다는 신호로 읽으면 된다.
