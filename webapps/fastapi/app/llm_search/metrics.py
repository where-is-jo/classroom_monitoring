...
# **1초 부근이 촘촘한 이유는 실측했기 때문이다.** GPU 서버(gemma 9B Q4_K_M, ctx 4096)에
# 실제 프롬프트로 12번 물어본 결과가 0.94~1.05초에 몰렸다(중앙값 0.96).
#
# 처음에는 상한 20초(타임아웃 기본값) 부근을 촘촘히 잡고 그 아래를 0.5·1·2초로 뒀는데,
# 그러면 관측값이 전부 le=1.0과 le=2.0 두 칸에만 들어간다. histogram_quantile은 칸
# 안에서 선형보간하므로 **실제 1.05초를 1.9초쯤으로 보고한다.** 지연 패널이 거짓말을
# 하게 되어 경계를 다시 잡았다.
#
# 꼬리를 60초까지 남기는 것은 그대로다. 카메라가 늘면 프롬프트가 길어지고, 규격 위반으로
# 재시도가 걸리면 두 배가 되며, 이 GPU는 다른 작업과 나눠 쓴다.
_LLM_BUCKETS = (0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 2.0, 3.0, 5.0, 10.0, 20.0, 60.0)
