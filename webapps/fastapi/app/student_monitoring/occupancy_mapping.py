...
def unseated_identities(
    ...
    **쓸 수 있는 좌석 ROI가 하나도 없으면 아무것도 돌려주지 않는다.** 좌석 지도가 없는
    카메라에서는 모든 탐지가 `NO_MATCH`이고, 그것을 "좌석 밖에 있다"로 읽으면 ROI를
    등록하지 않았다는 사실이 "자리에 없다"는 판정으로 둔갑한다. 실제로 자리에 앉아 있는
    학생이 강의실을 서성이는 것으로 기록된다.
