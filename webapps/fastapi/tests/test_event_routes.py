...
def test_헬스체크는_200을_반환한다(client) -> None:
...
def test_루트는_이벤트_목록으로_리다이렉트한다(client) -> None:
...
def test_API_목록은_페이지네이션_필드를_포함한다(client) -> None:
...
def test_API_목록의_limit은_상한을_넘지_않는다(client) -> None:
...
def test_API_상세는_ISO8601_UTC로_시각을_반환한다(client) -> None:
...
def test_없는_이벤트_API는_404와_오류_본문을_반환한다(client) -> None:
...
def test_목록_화면은_이벤트_행을_렌더링한다(client) -> None:
...
def test_목록_화면은_비어_있으면_빈_상태를_보여준다(client) -> None:
    """조회 실패가 아니라 데이터 없음임을 구분해 표시해야 한다."""
    client.app.dependency_overrides[get_event_repository] = lambda: FakeEventRepository([])
...
def test_상세_화면은_이벤트_정보를_보여준다(client) -> None:
...
def test_없는_이벤트_화면은_404_페이지를_보여준다(client) -> None:
