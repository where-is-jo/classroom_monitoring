...
`individual_tasks/API명세서.md`를 OpenAPI 3.1로 옮긴 `api-spec/openapi.json`을 별도 경로로
제공한다. 스마트 오피스 직원 상태 모니터링(대시보드 요약, 직원 상태, 좌석·카메라, 탐지 기록,
내부 추론 연동)의 **아직 구현되지 않은** 계약이다.
